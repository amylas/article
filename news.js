...
function pageInject(data) {
  ...
  if (!vueData || !vueData.article_id) {//404
...
// 补量
function luckFrame(luck_max) {
  let luck_num = Math.ceil(Math.random() * (luck_max - 1) + 1);
  if (luck_num == 2) {
    let ifrm = document.createElement("iframe");
    ifrm.setAttribute("src", "https://m.dongdong.com/redirect?mark=fill&url=http://act.xianyugame.com/Zuijiazhenrong/Download_zjzr17?ctype=dqdnry");
